...
$data = mysqli_query($koneksi, "SELECT * FROM kkn  WHERE id_kkn = '$id'");
while ($d = mysqli_fetch_array($data)) {
    $html .= ' <font size="3"><b>Nomor : ' . $d['no_surat'] . ' </b></font>';
};
$html .= '</center>
                </td>
            </tr>
...
while ($d = mysqli_fetch_array($data)) {
    $html .= ' 
        <tr style="border-bottom: 1px solid black">
            <td>Pada tanggal</td>
            <td>:</td>
            <td colspan="2">' . $d['tgl_dikeluarkan'] . '</td>
        </tr>';
};
$html .= '
        <tr>
...
file_put_contents('surat_izin/surat-izin-' . $id . '.pdf', $output);
echo "<script>alert('Surat Berhasil Dibuat')</script><meta http-equiv='refresh' content='2;url=pendaftaran_kkn1.php'>";
