            ...
            <div id="content">
                ...
                <!-- End of Topbar -->
                 ...
                 <div class="container-fluid">
                    ...
                    <!-- Page Heading -->
                    <h1 class="h3 mb-3 text-gray-800">Usulan di <b>Kelurahan</b> </h1>

                    <form action="proses-kirim-usulan.php" method="post">
                                ...
                                <select name="id_potensi" id="id_potensi" class=" form-control" tabindex="" >
                                    <option value=""></option>
                                            ...
                                            /*mengambil nama-nama kecamatan*/
                                                include "../koneksi.php";
                                                $query = mysqli_query($koneksi, "SELECT * FROM potensi JOIN potkel ON potensi.id_potensi = potkel.id_potensi JOIN kelurahan ON potkel.id_kel = kelurahan.id_kel JOIN user on user.id_user= kelurahan.id_user WHERE potensi.id_potensi != 0 and user.id_user='".$_SESSION['id_user']."'") or die (mysqli_error($koneksi));
                        ...
                        <div class="form-group">
                            <label for="potensi">Subpotensi:</label>
                                <select name="id_subpotensi" id="id_subpotensi" class=" form-control" tabindex="">
                                    <option value=""></option>
                                            
                                            <?php
                                            /*mengambil nama-nama kecamatan*/
                                                include "../koneksi.php";
                                                $query = mysqli_query($koneksi, "SELECT * FROM potkel JOIN potensi ON potensi.id_potensi = potkel.id_potensi JOIN kelurahan ON potkel.id_kel = kelurahan.id_kel JOIN user on user.id_user= kelurahan.id_user JOIN subpotensi ON subpotensi.id_potensi = potensi.id_potensi WHERE potensi.id_potensi != 0 and user.id_user='".$_SESSION['id_user']."'");
                                                while ($data = mysqli_fetch_array($query)) {
                                                    echo "<option value=\"$data[id_potensi]\" >$data[nama_subpotensi]</option>\n";
                                                }
                                            ?>
                                </select>
                        </div>
                        <div class="form-group">
                            <label for="potensi">Usulan:</label>
                            <textarea id="usulan" name="usulan" class="form-control" rows="4" cols="50" required></textarea>
                        </div>
